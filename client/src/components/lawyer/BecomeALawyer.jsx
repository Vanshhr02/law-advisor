...
function BecomeALawyer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send lawyer details to the admin for approval
      const response = await newRequest.post("/adminRouter/submit-lawyer", formattedData);
  
      if (response.status === 200) {
        setSuccess(true);
        setFormData({ name: "", contact: "", speciality: "", description: "" });
  
        toast.success("Form submitted! Pending admin approval.", {
          position: "top-center",
          autoClose: 3000,
        });
      }
